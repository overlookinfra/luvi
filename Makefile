...
test: luvi
	...
	rm -f test.bin
test-service: luvi
	rm -f test-service.bin
	LUVI_APP=samples/test-service.app build/luvi 1 2 3 4
	LUVI_APP=samples/test-service.app LUVI_TARGET=test-service.bin build/luvi
	LUVI_app= ./test-service.bin 1 2 3 4
	rm -f test-service.bin
install: luvi
